 ...
 * The rset, rho, path-key RSpace, cost-accounting, and knotted-topoi manuscripts
   ...
   `Rho.KMachine.ordinaryReceive_to_rho`, `Rho.KMachine.persistentReceive_to_rho`,
   ...
   kernels, the K ordinary and persistent receive cell steps, and the packet-level base-rewrite bridge
   that those constructions must instantiate.
