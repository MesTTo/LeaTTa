...
(`alphaEq`, in `Core/Alpha.lean`), faithful to Hyperon's `atoms_are_equivalent`.

This module proves the central fact and two structural consequences:
...
## The Boolean decider and host floats (a faithful caveat)
...
Boolean `alphaEq`, and this is deliberate. `Atom` contains `Ground.float : Float → Ground`, and
...
Boolean is the correct decision procedure for `AlphaEq` on the float-free fragment, and the
propositional relation is the right mathematical object: it is an equivalence relation
unconditionally. Hyperon inherits the same host-float behavior, so this is faithful to the
implementation rather than a divergence from it.
-/

namespace Metta

/-- Propositional α-equivalence: two atoms are α-equivalent iff they share a canonical form, i.e.
are equal up to a consistent renaming of their (atom-global, unscoped) query variables. The
kernel's Boolean `alphaEq` decides this relation, modulo host-float equality (see the module
docstring). -/
